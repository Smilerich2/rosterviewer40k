    ...
    <div className="mb-4 rounded-xl overflow-hidden">
      {ds.models.map((m, i) => (
        <div key={i} className={i > 0 ? 'mt-1' : ''}>
          ...
          <div className={`${theme.secondary} px-2 py-2.5 grid grid-cols-7 gap-1`}>
            {[
              { label: 'M',   value: m.m,           inv: false },
              { label: 'T',   value: m.t,           inv: false },
              { label: 'SV',  value: m.sv,          inv: false },
              { label: 'INV', value: m.invSv ?? '–', inv: true  },
              { label: 'W',   value: m.w,           inv: false },
              { label: 'LD',  value: m.ld,          inv: false },
              { label: 'OC',  value: m.oc,          inv: false },
            ].map(({ label, value, inv }) => {
              const hasInv = inv && !!m.invSv;
              return (
                <div
                  key={label}
                  className={`flex flex-col items-center justify-center rounded-md py-1.5 sm:py-2 ${
                    hasInv ? 'bg-amber-400 dark:bg-amber-500' : inv ? 'bg-white/20' : 'bg-white dark:bg-slate-100'
                  }`}
                >
                  <span className={`text-[8px] sm:text-[9px] font-bold uppercase tracking-wide leading-none mb-0.5 ${
                    hasInv ? 'text-amber-900' : inv ? 'text-white/50' : 'text-slate-400'
                  }`}>
                    {label}
                  </span>
                  <span className={`font-black text-sm sm:text-base leading-none tabular-nums ${
                    hasInv ? 'text-amber-900' : inv ? 'text-white/40' : 'text-slate-900'
                  }`}>
                    {value}
                  </span>
                </div>
